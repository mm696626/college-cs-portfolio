...
public class Board {
    ...
    public boolean doMove ( int fromRow, int fromCol, int toRow, int toCol ) {
        if(isValidMove(fromRow,fromCol,toRow,toCol) == true && gameOver() == false)
        {
            if(bluesTurn==true)
                bluesTurn=false;
            else
    ...
    // version 4
    //=======================================================================
    // returns the number of blue pieces remaining on the board.
    // (accomplish this by scanning the entire board while counting the blue pieces.)
    public int countBlue ( ) {
        ...
        for(int i=0; i<piece.length; i++)
        {
            for(int j=0; j< piece[0].length; j++)
            {
                if(getColor(getPiece(i,j)) == GameColor.Blue)
                {
                    count++;
                }
    ...
    public int countRed ( ) {
        ...
        for(int i=0; i<piece.length; i++)
        {
            for(int j=0; j< piece[0].length; j++)
            {
                if(getColor(getPiece(i,j)) == GameColor.Red)
                {
                    count++;
                }
    ...
    public boolean isRedWinner ( ) {
        if(getColor(getPiece(8,3)) == GameColor.Red) //check if piece at blue den is red
            return true;
        else
            return false;
    }
    //-----------------------------------------------------------------------
    // returns true if blue is a winner (regardless of whose turn it is).
    // false otherwise.
    // (accomplish this by checking if a blue piece is in the red den.)
    public boolean isBlueWinner ( ) {
        if(getColor(getPiece(0,3)) == GameColor.Blue) //check if piece at red den is blue
            return true;
        else
            return false;
    }
    ...
    public Board ( final Board original ) {
        //copy elements from piece array (not just give memory address)
        for(int i=0; i<piece.length; i++)
        {
            for(int j=0; j<piece[0].length; j++)
            {
        ...
        //copy elements from board array (not just give memory address)
        for(int i=0; i<board.length; i++)
        {
            for(int j=0; j<board[0].length; j++)
            {
    ...
    public boolean equals ( Object other ) {
        if(other == null || this == null) //no point in comparing null objects
            return false;

        if(this == other) //check if two board objects are identical (by memory address)
            return true;


        if(this.getClass() == other.getClass()) //see if both objects are of the same class (so Board class in this case)
        {
            //create a object to use the submethod equalsBoard
            Board compareBoard = (Board) other; //comparison board object
            if(this.bluesTurn == compareBoard.bluesTurn) //check if bluesTurn state is the same as it isn't checked in equalsBoard
            {
                if(equals(compareBoard.piece) == true) //run equals and see if board pieces are equal
                    return true;
            }

        }

        return false; //if all tests fail return false (I needed to return something)
    }
    ...
    public boolean equals ( Piece[][] otherPiece ) {
        for(int i=0; i<piece.length; i++)
        {
            for(int j=0; j<piece[0].length; j++)
            {
                if(piece[i][j] != otherPiece[i][j])
                {
                    return false; //check if piece is not equal return false
                }
            }
        }
    ...
    // when is the game over? the game is over when:
    //     a. blue is a winner, or
    //     b. red is a winner,  or when
    //     c. blue or red don't have any pieces left.
    public boolean gameOver ( ) {
        // \todo v5
        if(isBlueWinner() == true || isRedWinner() == true || countBlue() == 0 || countRed() == 0) //check all conditions if the game is over
        {
            return true;
        }
        else
    ...
    //  *** this seems to have disappeared/changed, so use the pdf below. ***
    //
    //  simply treat the individual piece values (i.e., piece[r][c].ordinal()) as the
    //  individual int values. (DO NOT use charAt.) also include the value of bluesTurn
    //  as described below as the last (i.e., nth) int.
    //
    //  DO NOT include anything else (just the piece array contents and bluesTurn).
    @Override
    public int hashCode ( ) {
        ...
        for (int r = 0; r < Board.fRows; r++) {
            for (int c = 0; c < Board.fCols; c++) {
                ...
                hash += piece[r][c].ordinal(); //get int value of individual pieces

                i++;
            }
        }
        //include bluesTurn as the last character value. use 1 if it
        // is true, or 2 if it is false. (use the int values 1 and 2,
        // not the char values, '1' and '2'.

        if(bluesTurn == true) //if blues turn is true increment by 1 else 2
            hash+=1;
        else
            hash+=2;
   ...
   //Don't forget to modify doMove to call gameOver. You shouldn't be allowed to make moves after the game is over.


    //version 6:
    // create a list of all possible moves given 'this' as the starting position.
    // this is basically a list of possible or "suggested" moves.
    public ArrayList< Board > suggest ( ) {
        ArrayList< Board > list = new ArrayList< Board >();
        list.add(this); //add starting board
        Board copy = new Board(this); //copy calling board
        //call doMove with calling object with each square (this is very brute force I'm aware)
        for(int i=0; i<Board.fRows; i++)
        {
            for(int j=0; j<Board.fCols; j++)
            {
                for(int k=0; k<Board.fRows; k++)
                {
                    for(int l=0; l<Board.fCols; l++)
                    {
                        if(copy.doMove(i,j,k,l) == true)
                        {
                            copy = new Board(copy); //copy the copy to a new board
                            list.add(copy);
    ...
    //version 7:
    // call suggest() and choose the best from the list.
    // to start, you may choose one at random (but you will NOT receive any credit for that approach).
    public Board chooseBest ( ) {
        ArrayList< Board > list = suggest();

        //todo: add your code here. call h for each element in list and pick the best.

        return best;
    }

    //version 7:
    // this function evaluates the current board configuration, b, and returns a value in [-inf, ..., +inf]
    // where +inf indicates a winner, -inf is a loser, and values in between are better (greater than) or worse (less than)
